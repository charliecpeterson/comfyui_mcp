...
2. **Optical-Realism depth-aware post-pass** — `ComfyUI-Optical-Realism`. One physics node that replaces separate grain/CA/vignette: depth-of-field falloff, atmospheric haze, lifted blacks, highlight roll-off, halation, film grain, chromatic aberration. Needs a depth map: `DepthAnythingV2Preprocessor` (controlnet_aux) → its IMAGE into `OpticalRealism.depth_map`, original image into `OpticalRealism.image`.
   ...
   - **When to apply (validated across scenes):** the post-pass's value scales with scene *depth* + *dynamic range*. Bright scene with clear depth planes (e.g. a window) → big win. Moody, gradual depth → moderate. **Flat, top-down, already detail-rich images (especially Z-Image output) → near-wash, and the grain/DOF slightly soften the micro-detail — skip it or go subtle-only** (DOF off, `grain_power` ~0.01, keep just warmth + highlight roll-off). The full stack is not always better than the generation alone.
