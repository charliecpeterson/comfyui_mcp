...
2. **Optical-Realism depth-aware post-pass** — `ComfyUI-Optical-Realism`. One physics node that replaces separate grain/CA/vignette: depth-of-field falloff, atmospheric haze, lifted blacks, highlight roll-off, halation, film grain, chromatic aberration. Needs a depth map: `DepthAnythingV2Preprocessor` (controlnet_aux) → its IMAGE into `OpticalRealism.depth_map`, original image into `OpticalRealism.image`.
   ...
   - **When to apply — depth decides it (validated across scenes).** The mechanic: the depth-aware DOF blurs *only the far background*. So:
     - **Scene with a clear foreground subject + receding background** (a table → a street, a person → a room): the blur lands on the background, the subject stays sharp and *gains* lens separation → **apply the full pass.** Big win even in flat overcast light — depth, not brightness, is what drives it.
     - **Flat / top-down / subject-fills-the-frame scene** (no background to recede): nothing for the DOF to blur, so the grain + global passes hit the *subject* and soften its micro-detail → **skip it, or go subtle-only** (DOF off, `grain_power` ~0.01, keep only warmth + highlight roll-off).
     - Bottom line: **depth matters more than dynamic range**, and the full stack is *not* always better than the generation alone. Flat, already-detail-rich output (especially Z-Image) is usually best left raw.
...
Reusable templates live in the workflow library: `run_workflow(path="utils/optical-realism-postpass.api.json")` for the post-pass, and the generators `run_workflow(path="krea2/krea2_t2i_detail-daemon.api.json")` / `run_workflow(path="zimage/zimage_t2i_detail-daemon.api.json")`. **Note:** `MarkdownNote` is frontend-only — never put one in an API-format workflow (it breaks headless `/prompt`); use a sidecar `.md` instead.

**Model pick (validated head-to-head, same prompt + Detail Daemon):** for candid photo-realism **Z-Image Turbo wins** — documentary flatness + the strongest skin/hand micro-detail of the local models (and only 4 steps). **Krea 2** renders moodier/more cinematic — pick it when you want an artful "photographer's photo" over a raw snapshot. Both beat Flux/Qwen/HiDream out of the box for this. Z-Image needs `ModelSamplingAuraFlow(shift=3)`, `EmptySD3LatentImage`, the `qwen_3_4b` encoder (CLIPLoader type `lumina2`), and `res_multistep`/`simple`; prefer a GGUF/fp8 build over bf16 to cut its slow cold-load.
