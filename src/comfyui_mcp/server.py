...
@mcp.tool()
def generate_workflow_index(dir: str = "", filename: str = "_index.md") -> dict[str, Any]:
    """Regenerate a browsable Markdown catalog of the workflow library, written to
    <dir>/<filename> (default <COMFYUI_ROOT>/user/default/workflows/_index.md).

    Walks every .json workflow, pulls its main model, key node types, and first
    in-canvas note (same describe path catalog_workflows uses), and writes a table
    grouped by top-level folder. Run after adding/renaming workflows to keep the
    index current — it's the one library artifact that goes stale on every save.

    Args:
        dir: workflow root to index. Empty = the default workflows dir.
        filename: markdown file to write inside `dir`.

    Returns: {root, index_path, workflows_indexed, folders}.
    """
    from collections import defaultdict
    base = Path(dir) if dir else _comfy_root() / "user" / "default" / "workflows"
    if not base.is_dir():
        return {"ok": False, "error": "workflows dir not found", "root": str(base)}

    by_folder: dict[str, list[dict[str, Any]]] = defaultdict(list)
    count = 0
    for jf in sorted(base.rglob("*.json")):
        if not jf.is_file() or "@eaDir" in str(jf) or jf.name == filename:
            continue
        rel = jf.relative_to(base)
        folder = rel.parts[0] if len(rel.parts) > 1 else "(root)"
        summary = _describe_workflow_file(str(jf), summary_only=True)
        model = next((Path(r["value"]).name for r in (summary.get("model_references") or [])
                      if r.get("value")), "")
        nodes = ", ".join((summary.get("top_types") or [])[:4])
        note = ""
        for n in (summary.get("notes_in_canvas") or []):
            t = (n if isinstance(n, str) else (n.get("text", "") if isinstance(n, dict) else "")) or ""
            t = t.strip().replace("\n", " ")
            if t and not t.startswith("http"):
                note = t[:90]
                break
        name = str(rel.relative_to(folder)) if folder != "(root)" else rel.name
        by_folder[folder].append({"name": name, "model": model, "nodes": nodes, "note": note})
        count += 1

    lines = ["# Workflow index\n",
             f"{count} workflows, grouped by folder. Auto-generated by generate_workflow_index — "
             "re-run after adding or renaming workflows.\n"]
    for folder in sorted(by_folder, key=lambda f: (f == "(root)", f)):
        rows = by_folder[folder]
        lines.append(f"\n## {folder}/  ({len(rows)})\n")
        lines.append("| workflow | model | key nodes | note |")
        lines.append("|---|---|---|---|")
        for r in sorted(rows, key=lambda x: x["name"]):
            lines.append(f"| `{r['name']}` | {r['model']} | {r['nodes']} | {r['note'].replace('|', chr(92)+'|')} |")
    out = base / filename
    out.write_text("\n".join(lines))
    return {"root": str(base), "index_path": str(out), "workflows_indexed": count,
...
def describe_model(path: str, category: str = "") -> dict[str, Any]:
    ...
    if Path(path).is_absolute():
        ...
        # Resolve across every model search root (local models/ + extra_model_paths
        # /NAS), not just the local tree — so describe_model sees the same files
        # list_models/find_model do. Split off the category to query its search dirs.
        if category:
            cat, rest = category, path
        else:
            parts = Path(path).parts
            cat, rest = (parts[0], str(Path(*parts[1:]))) if len(parts) > 1 else ("", path)
        full = (_comfy_root() / "models" / path).resolve()
        if cat:
            for d in _model_search_paths(cat):
                cand = d / rest
                if cand.is_file():
                    full = cand.resolve()
                    break
...
async def run_workflow(
    ...
    Pass `path` to load and run a saved API-format workflow file from disk (e.g. a
